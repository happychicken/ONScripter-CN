...
public class AudioThread {
	...
	public AudioThread(MainView parent)
	{
		mParent = parent;
		mAudio = null;
		mAudioBuffer = null;
	...
	public int fillBuffer()
	{
		...
		{
			//if( Globals.AUDIO_BUFFER_CONFIG == 0 ) // Gives too much spam to logcat, makes things worse
			//	mAudio.flush();
	...
	public int initAudio(int rate, int channels, int encoding, int bufSize)
	{
			if( mAudio == null )
			{
					...
					if(Globals.AUDIO_BUFFER_CONFIG != 0) {    // application's choice - use minimal buffer
						bufSize = (int)((float)bufSize * (((float)(Globals.AUDIO_BUFFER_CONFIG - 1) * 2.5f) + 1.0f));
						mVirtualBufSize = bufSize;
					}
