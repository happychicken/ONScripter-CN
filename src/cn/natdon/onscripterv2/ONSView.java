...
public class ONSView extends Activity{
	...
	public void VirtualButton() {
		...
		Button btnright = new Button(this); // �Ҽ�
		...
		btnright.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				mView.nativeKey(KeyEvent.KEYCODE_ESCAPE, 1);
				mView.nativeKey(KeyEvent.KEYCODE_ESCAPE, 0);
			}
		});
		...
		/*
		Button btnvolup = new Button(this); // Vol+
		...
		btnvolup.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				mView.nativeKey(KeyEvent.KEYCODE_VOLUME_UP, 1);
				mView.nativeKey(KeyEvent.KEYCODE_VOLUME_UP, 0);
			}
		});
		...
		Button btnvoldown = new Button(this); // Vol-
		...
		btnvoldown.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				mView.nativeKey(KeyEvent.KEYCODE_VOLUME_DOWN, 1);
				mView.nativeKey(KeyEvent.KEYCODE_VOLUME_DOWN, 0);
			}
		});
		...
		btngetshot.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				mView.ShowToast(getResources().getString(R.string.Screenshot_Tip)+Globals.CurrentDirectoryPath);
	...
	public void BtnCancel() {
		if (popbtn != null && popbtn.isShown()) {
			...
			VirtualButton();
			mView.ShowToast(this.getString(R.string.VirtueButton_Tip));
		}
	}
		...
		try  
		   {   
			    ...
			    nowtime = df.format(new Date());
		            ...
		            myTime=this.getString(R.string.Game_Process)+hour+this.getString(R.string.Game_Hours)+min+this.getString(R.string.Game_Minutes)+"\n"+this.getString(R.string.Game_CurTime)+ctime+"\n"
		            		+this.getString(R.string.Game_AvailMemory)+getAvailMemory()+"\n"+battery;
		}   
		        catch (Exception e)   
		        {   
		        } 
	...
	public BroadcastReceiver mBatteryInfoReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (Intent.ACTION_BATTERY_CHANGED.equals(action)) {
				...
				battery = getResources().getString(R.string.Game_Battery)+String.valueOf(level * 100 / scale) + "%";
			}
		}
	};
	...
	{
		showTaskbarNotification("MiNE���ں�̨����", "MiNE "+getApplicationVersion(), "MiNE���ں�̨����, ����ָ���");
	}
