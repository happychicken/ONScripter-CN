...
public class PlayerActivity extends Activity implements
		...
		mEventHandler = new Handler() {
			public void handleMessage(Message msg) {
				...
				case MEDIA_PLAYER_VIDEO_SIZE_CHANGED: {
					...
					SurfaceView surface = mSurfaceViewDef;
					int ar = mAspectRatio;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		//initializeData();
		Intent in=getIntent(); 
		String videofile=in.getStringExtra("one"); 
		Vmode = in.getBooleanExtra("two",false);
		mPlayListSelected = 0;
		mPlayListArray = new ArrayList<String>();
	...
	public void onClick(View v) {
		if (!mMediaPlayerLoaded)
			return;
		int id = v.getId();
		if (id == R.id.player_button_switch_audio) {
		} else if (id == R.id.player_button_switch_subtitle) {
		} else if (id == R.id.player_button_previous) {
		} else if (id == R.id.player_button_toggle_play) {
		...
		} else if (id == R.id.player_button_switch_aspect_ratio) {
	...
	public void onStopTrackingTouch(SeekBar seekBar) {
		...
		if (id == R.id.player_seekbar_progress) {
